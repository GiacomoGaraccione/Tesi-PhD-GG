...
\GG{la definizione di serious games va nel background}
...
With the literature offering essentially no gamified tool for teaching BPMN modeling practices and two serious games that offer limited ability for students to solve exercises on their own, without relying on predefined structures and elements to select, a need for a gamified tool that offers a free modeling environment was identified, and thus one of the first activities of the PhD was to develop a prototype for such a tool.
...
\textbf{Rewards}, a theme centered around giving prizes to users in case of successful task completion. For the prototype version, an intangible reward was selected in the form of a jigsaw puzzle to be completed, with pieces of the puzzle being unlocked by completing exercises, with harder exercises giving more pieces. Lastly, to support the rewarding aspect, exercises have also been made unlockable, requiring users to complete easier exercises before more challenging ones can be tackled.

A summary of the elements that have been implemented for each theme, together with a brief description and the corresponding Core Drive in the Octalysis framework, is presented in Table \ref{tab:proto_elements}. \GG{descrizione di Octalysis da aggiungere nel background, nomi dei vari CD consistenti nei diversi capitoli}
...
Stakeholders & Developers, researchers, teachers \\ \bottomrule
\end{tabular}
\label{tab:gqm1}
\end{table}
...
Participants were selected via convenience sampling among students of the Masters' degree in Computer Engineering at Politecnico di Torino as representative of students that would learn BPMN during their study career; due to the trial being a preliminary study, a small number of participants was selected, with a total of 12 volunteering students being involved. Participation in the study was rewarded with tangible prizes such as free snacks offered by the thesis student who was overseeing the development of the prototype, Ms. Kylie Bedwell. \GG{scrivere da qualche parte nell'inizio di sto capitolo che è partita come tesi (che ho seguito io?)}

Since the trial consisted of comparing three different implementation of the BIPMIN prototype, the study design needed to ensure that participants could try all three versions to conduct fair judgement; it was thus not possible to adopt a between-subjects technique where each participant would try only one of the three versions of the prototype and there would be no way to compare the differences in game mechanics. For this purpose, a within-subjects design was selected, meaning that participants performed one of the three modeling tasks (T3, T4, and T5) for each version of the prototype; furthermore, the use of this design is also more suited for experiments with a small sample of participants, making it the better choice for the pilot study.

A limitation of the within-subject design, however, is the fact that learning biases could be in place, with participants learning how to use the application in the first task and thus perform better in subsequent ones. To address this limitation, tasks were counter-balanced by having them be performed in different order by different participants, meaning that different versions of the prototype would have been experienced in different order; tasks T3, T4, T5 were then arranged with a balanced Latin square to ensure that the order was not just the one presented earlier (progress, then competition, then rewards), but that a total of 6 different variations of tasks were possible, resulting in each variation being used twice, given the total of 12 participants. The six combinations of tasks according to the Latin square can be seen in Table \ref{tab:latinsquare}.
...
To answer RQ2, which focused on assessing the usability of the BIPMIN prototype, a standardized method was used in the form of the System Usability Scale survey, which was supplied to the participants as part of the post-study questionnaire. The SUS survey is commonly used to provide a measure of software applications usability, consisting of 10 statements focused on the experience on using an application with answers in the form of a Likert scale ranging from 1 (Strongly Disagree) to 5 (Strongly Agree). Statements alternate between positive and negative to counteract possible biases where responders select answers without considering the actual content of the question they are ranked.

Once all questions are answered, the final score is computed as the measure of perceived usability of the system under evaluation in a range between 0 and 100; conventionally, scores above 68 imply that the application has an above average usability. The score is computed using the following equation:
\begin{equation}
Score = 2.5 ( \sum(Q_{odd} - 1)  + \sum(5 - Q_{even}) ) 
\label{eq:sus}
\end{equation}

where $Q_{odd}$ is the score obtained by positive-connotated statements, while $Q_{even}$ is the score for negative ones. \\
...
Lastly, the survey included an open question where participants could leave comments regarding feedback or suggestions to improve the prototype: only half of the participants answered this question, and their comments included suggestions such as the inclusion of a BPMN tutorial, the addition of an achievement system, the creation of a one-on-one competition mode, and the conviction that the leaderboard could be useful in a classroom environment, despite the participant themselves noting they were not a competitive person.
...
\textbf{Threats to External Validity} discuss factors that affect the generalizability of the results. This study was designed as a preliminary evaluation of a prototype gamified educational tool for process modeling focused specifically on BPMN. The results cannot be generalized to other process modeling notations, to non-educational context where BPMN can be employed, or to educational contexts focused on other topics. Furthermore, the results cannot also be generalized depending on the selected game elements and mechanics, meaning that results cannot be compared to those that other approaches, focused on different game mechanics, could obtain.

\textbf{Threats to Construct Validity} relate to how much the metrics selected for analysis represent the observed phenomenon. For this study, the metrics that have been measured (e.g., enjoyment and motivation scores, time spent completing tasks, preferred versions and mechanics) have not been mapped to the effectiveness of BIPMIN as an educational tool, meaning that its effectiveness on improvement of student performance is still not known.
...
Stakeholders & Researchers, teachers \\ \bottomrule
\end{tabular}
\label{tab:gqm1}
\end{table}
...
After settling on the experiment design and how to administer the treatment, the number of tasks was finalized as a total of two exercises, one per each treatment: this resulted in a 2x2 full factorial crossover experiment where, depending on the combination of both the treatment (Gamified-then-Vanilla or Vanilla-then-Gamified) and task (Exercise 1 followed by Exercise 2 or vice-versa), a total of four possible experimental conditions could be possible; the four possible combinations defined four possible groups and are presented in Table \ref{tab:exp_design}. To ensure fairness, participants were randomly assigned to the four groups, ensuring that each group had a total of 50 participants.
...
For what concerns RQ2, \textit{Correctness} was split into \textit{Syntax correctness} and \textit{Semantic correctness}. The former was measured as the percentage of syntax rules satisfied by a diagram over the total number of syntax rules, while the latter was computed as the number of process parts modeled by a diagram over the total number of parts. \GG{scrivere poi ste regole}

RQ3 related to the students' perception of the gamified experience, which was assessed with the GAMEX questionnaire. This questionnaire evaluates gamified experiences with questions grouped around six constructs: Enjoyment, Absorption, Creative Thinking, Activation, Possible Negative Effects, Dominance, with answers being expressed as a Likert scale ranging from 1 - \textit{Strongly Disagree} to 5 - \textit{Strongly Agree}. %The full list of questions is visible in Tables \ref{tab:q12}, \ref{tab:q34}, and \ref{tab:q56} of Appendix \ref{app:questionnaire}.
...
After collecting all metrics, it was decided to adopt a non-parametric approach to test the hypotheses, assuming that most variables were not normally distributed. Given that the experiment was designed as a full factorial crossover study, the data was analyzed using a repeated measures linear mixed models, according to the guidelines by Vegas et al. \cite{CrossoverDesign}; furthermore, the two exercises and the order of treatment have also been considered design factors during the analysis, to counteract possible threats to validity related to the design.

RQ3 and RQ4, instead, were evaluated through a questionnaire whose answers could not be analyzed with statistical hypothesis testing; the answers were analyzed through descriptive statistics, with a focus on the distribution of scores for Likert-scale questions (RQ3 and RQ4.1). For these questions, the analysis method consisted of computing the average score assigned to each set of questions as the overall opinion for each student, and then plotting the distribution. Lastly, RQ4.2 and RQ4.3 were answered with open-ended questions where students could present their opinions: these answers were analyzed by adopting the Straussian Grounded Theory approach, with the \textit{open coding} strategy being selected, since the goal of the analysis for the two research questions was to identify themes in the answers.
The open coding procedure, which was performed according to existing guidelines for how to apply grounded theory to software engineering \cite{grounded}, consisted of analyzing each single answer, trying to identify its main topic, and looking among the set of already identified topics to determine the most compatible one with the given one. In case no topic was found in the set, the  newly defined topic would be added to the set, and the answer would be tagged with it; otherwise, the answer would be tagged with the existing suitable topic, leading to the creation of an incrementally growing set.

After all answers had been tagged with a topic, a second pass was performed on all answers and topics, to identify answers that could potentially be assigned a different topic; following this step, a third pass was done to identify answers that could have more that one suitable topic, leading to the assignment of multiple topics to some answers.
...
\begin{table}[ht]
  ...
  \label{tab:tabrq1_anova}
    \begin{tabular}{lcccc}
         \toprule
         Factor / vars:& 
         \multicolumn{2}{c}{Size (${H_{s_{0}}}$)} & \multicolumn{2}{c}{Submissions (${H_{as_{0}}}$)} \\
...
The first point of interest observed was the impact on student performance, measured by observing the size of the saved diagrams and the number of times students submitted their diagrams for grading to the automated evaluation engine: while size was not impacted in any way by the use of gamification, it emerged that students were more willing to perform evaluations when using the gamified version, as that version tied game mechanics to the results of the automated checks. It can be deducted that, while gamification does not cause significant changes in the students' modeling habits such as adding or removing multiple elements, it can be beneficial by guiding them toward checking more frequently whether their solutions are correct or not.
