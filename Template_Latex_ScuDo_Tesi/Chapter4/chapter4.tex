...
This chapter describes the design of the new framework and its application in the development of a tool named UMLegend, a web platform developed from spring 2023 for gamifying UML class diagram modeling; furthermore, the chapter also discusses the empirical experiment that was performed to evaluate UMLegend's impact on student performance, as well as how a tool implemented according to the new framework would be perceived compared to the original BIPMIN implementation of certain game mechanics. The UMLegend tool was first presented in \cite{cagnazzo_2023_umlegend}, while the empirical experiment and its results were described in both \cite{garaccione_2024_sqj} and \cite{garaccione_2026_gamification}; a Docker container used to replicate the functionalities of the tool used during the experiment has been published online\footnote{\url{https://hub.docker.com/r/giacomogaraccione/umlegend_replication}}.
...
The final questionnaire consisted of multiple sets of questions, with each set being focused around a specific sub-question. To answer RQ3.1, which aimed at assessing the overall student perception of the gamified experience, questions taken from the Technology Acceptance Model (TAM) \cite{davis1989perceived, lee2003technology} and GAMEX \cite{eppmann2018gameful} questionnaires, both introduced in Section \ref{sec:empirical_methods}, were used, measuring the four TAM constructs and the six GAMEX dimensions using a Likert scale going from a score of 1 (Strongly Disagree) to 5 (Strongly Agree).
